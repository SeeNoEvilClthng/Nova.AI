...
function render(){
  ...
  $("agentCount").textContent=p?"3":"0";$("taskCount").textContent=String((state.executionTasks||[]).filter(task=>task.status!=="done").length);
...
function renderExecutionBoard(){
  ...
  container.className=tasks.length?"execution-list":"execution-empty";container.innerHTML=tasks.length?tasks.map(task=>`<article><span class="execution-state ${escapeHtml(task.status)}">${task.status==="done"?"✓ Done":task.status==="in_progress"?"● In progress":"○ Ready"}</span><div><small>${escapeHtml(task.department||"Company")} · ${escapeHtml(task.owner||"AI employee")}</small><h4>${escapeHtml(task.title)}</h4><p>${escapeHtml(task.handoff||"")}</p></div><button class="secondary" type="button" data-task-id="${escapeHtml(task.id)}">${task.status==="done"?"Reopen":task.status==="in_progress"?"Mark done":"Start task"}</button></article>`).join(""):`<b>No approved tasks yet</b><span>Approve an employee assignment to create an execution checklist.</span>`;
  container.querySelectorAll("[data-task-id]").forEach(button=>button.onclick=async()=>{const task=tasks.find(item=>item.id===button.dataset.taskId);if(!task)return;task.status=task.status==="todo"?"in_progress":task.status==="in_progress"?"done":"todo";task.updatedAt=new Date().toISOString();state.activities=state.activities||[];state.activities.push({title:task.status==="done"?"Approved task marked complete":task.status==="in_progress"?"Approved task started":"Approved task reopened",detail:task.title.slice(0,100)});await save();render();toast(task.status==="done"?"Task marked complete":"Task status updated");});
}

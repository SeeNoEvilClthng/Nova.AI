...
function renderEmployeeApprovals(){
  ...
  container.querySelectorAll("[data-workforce-approval]").forEach(button=>button.onclick=async()=>{const index=Number(button.dataset.approvalIndex),run=state.agentRuns[index];if(!run)return;if(button.dataset.workforceApproval==="changes"){const panel=container.querySelector(`[data-revision-panel="${index}"]`);panel.hidden=false;panel.querySelector("textarea").focus();return;}run.approvalStatus="approved";run.reviewedAt=new Date().toISOString();state.activities=state.activities||[];state.activities.push({title:"Employee assignment approved",detail:run.goal.slice(0,100)});await save();render();toast("Employee actions approved");});

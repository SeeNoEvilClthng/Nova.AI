...
function render(){
  ...
  $("agentCount").textContent=p?"3":"0";$("taskCount").textContent=p?(state.approved?"3":"1"):"0";
...
function notificationItems(){
  const items=[],runs=state.agentRuns||[],pending=runs.filter(run=>run.needsApproval&&!run.approvalStatus),queued=state.workQueue||[],usage=state.aiUsage||{},budget=state.aiBudget||{},personalLimit=Number(budget.tokenLimit||100000),planLimit=Number(window.novaEntitlement?.tokenCeiling||Infinity),limit=Math.min(personalLimit,planLimit),ratio=limit?Number(usage.totalTokens||0)/limit:0;
...
function renderEmployeeApprovals(){
  ...
  container.innerHTML=`<div class="approval-queue-head"><div><small>EMPLOYEE ACTIONS</small><h3>${pending.length} assignment${pending.length===1?"":"s"} awaiting review</h3></div><span>Nothing has been sent or purchased</span></div>${pending.map(({run,index})=>`<article><div><small>${escapeHtml(run.department||"company")} · ${new Date(run.createdAt).toLocaleDateString()}</small><h4>${escapeHtml(run.goal)}</h4><p>${escapeHtml(run.summary)}</p></div><div><button class="secondary" data-workforce-approval="changes" data-approval-index="${index}">Request changes</button><button class="primary" data-workforce-approval="approve" data-approval-index="${index}">Approve proposed actions</button></div></article>`).join("")}`;
  document.querySelectorAll("[data-workforce-approval]").forEach(button=>button.onclick=async()=>{const index=Number(button.dataset.approvalIndex),run=state.agentRuns[index];if(!run)return;run.approvalStatus=button.dataset.workforceApproval==="approve"?"approved":"changes_requested";run.reviewedAt=new Date().toISOString();state.activities=state.activities||[];state.activities.push({title:run.approvalStatus==="approved"?"Employee assignment approved":"Employee assignment returned for changes",detail:run.goal.slice(0,100)});if(run.approvalStatus==="changes_requested"){$("workforceGoal").value=`Revise this assignment with a narrower, more evidence-based approach: ${run.goal}`;}await save();render();toast(run.approvalStatus==="approved"?"Employee actions approved":"Assignment ready to revise");});
}
...
async function executeWorkforce(goal,department,priority,queuedId=null){
  ...
  button.disabled=true;if(queuedButton)queuedButton.disabled=true;button.textContent="Employees are working…";$("workforceStatus").textContent="Working";$("workforceResults").className="workforce-running";$("workforceResults").innerHTML='<i></i><b>Breaking the goal into specialist assignments</b><span>Strategy → Research → Product → Growth → Operations</span>';
  try{const response=await authFetch("/api/workforce/run",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({goal,department,priority,company:currentWorkspaceName,workspaceId:currentWorkspaceId,context:{plan:state.plan||null,evaluation:state.evaluation||null}})}),result=await response.json();if(!response.ok)throw new Error(result.error||"The workforce could not start");const run={...result.run,goal,department,priority,createdAt:new Date().toISOString()};recordUsage(run.contribution,"workforce");if(queuedId)state.workQueue=(state.workQueue||[]).filter(item=>item.id!==queuedId);state.agentRuns=[run,...(state.agentRuns||[])].slice(0,10);selectedRunIndex=0;state.activities=state.activities||[];state.activities.push({title:"AI workforce completed an assignment",detail:`${run.employees?.length||0} employees delivered work for: ${goal.slice(0,80)}`});await save();render();toast(run.contribution?.model==="Demo engine"?"Safe fallback completed the assignment":"Live AI employee run ready for review");}
  catch(error){renderWorkforce();toast(error.message);}finally{officeBusy=null;renderOffice();button.disabled=false;button.innerHTML='Assign to AI employees <span>→</span>';if(queuedButton?.isConnected)queuedButton.disabled=false;}
}

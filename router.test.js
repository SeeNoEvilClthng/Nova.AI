...
test("creates an approval-only campaign fallback from verified product facts", async () => {
  ...
  try{const result=await router.generateResellerCampaign({goal:"Introduce this item to collectors",audience:"fragrance collectors",callToAction:"DM us to purchase.",preferredStyle:"minimal",product:{name:"Sealed fragrance",category:"Fragrance",condition:"New",price:190,quantity:2,description:"Seller verified 70 mL bottle in its sealed original box."}});assert.equal(result.contribution.role,"Nova Campaign Copilot");assert.equal(result.contribution.model,"Demo engine");assert.match(result.campaign.caption,/Sealed fragrance/);assert.equal(result.campaign.style,"minimal");assert.equal(result.campaign.callToAction,"DM us to purchase.");assert.doesNotMatch(result.campaign.caption,/authentic/i);assert.ok(result.campaign.reviewWarnings.length>0)}finally{if(original.openai===undefined)delete process.env.OPENAI_API_KEY;else process.env.OPENAI_API_KEY=original.openai;if(original.gateway===undefined)delete process.env.AI_GATEWAY_API_KEY;else process.env.AI_GATEWAY_API_KEY=original.gateway;if(original.oidc===undefined)delete process.env.VERCEL_OIDC_TOKEN;else process.env.VERCEL_OIDC_TOKEN=original.oidc}
});

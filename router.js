...
function fallbackResellerCampaign(input) {
  const product=input.product||input,goal=String(input.goal||"").trim(),name=String(product.name||"Product").trim(),condition=String(product.condition||"").trim(),price=Number(product.price||0);
  const direction=goal||`Introduce ${name} using its seller-verified details.`;
  return {headline:`Now available: ${name}`.slice(0,100),message:direction.slice(0,240),caption:`${name} is now available${condition?` in ${condition.toLowerCase()} condition`:""}${price?` for $${price.toFixed(2)}`:""}. Review the seller-verified details before purchasing.`.slice(0,500),callToAction:"Message us for the verified details and availability.",style:"editorial",reviewWarnings:["Confirm the caption, price, availability, and creative before approving this draft."]};
}
...
async function generateResellerCampaign(input) {
  ...
  const warnings=[];let output=null;
  try{output=await runOpenAI(input,schema,{role:"Nova Campaign Copilot",schemaName:"reseller_campaign_draft",instructions:"You are Nova.Ai's Campaign Copilot. Turn the user's campaign goal into one polished social campaign draft using only the supplied seller-verified product facts. Never invent authenticity, scarcity, demand, discounts, performance, materials, provenance, shipping terms, reviews, or product details. Do not claim the content was approved, scheduled, posted, or sold. Keep the draft persuasive but truthful. Choose exactly one allowed visual style. Put facts that need human confirmation in reviewWarnings. All output remains a draft requiring the signed-in user's approval. Return one JSON object matching the schema."});}catch(error){warnings.push(error.message)}
